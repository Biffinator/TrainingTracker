...
test('parses folded, escaped all-day events with both duration formats',()=>{
 ...
 assert.deepEqual(ev.map(e=>e.date),['2026-09-07','2026-09-08','2026-09-08','2026-09-15']);
 assert.deepEqual(ev.map(e=>e.sport),['Strength and conditioning','Bike','Run','Run']);
 ...
 assert.equal(ev[3].minutes,90);   // 1:30:00
...
test('merge replaces the Endurance placeholder, adds two sessions, ignores strength, keeps Cold plunge',()=>{
 ...
 assert.equal(mergeAthletica(db,events,'2026-09-08'),1); // only 09-08 is inside the 7-day window
...
test('window is today through today+6 inclusive',()=>{
 const db=fresh();
 assert.equal(mergeAthletica(db,parseIcs(ICS),'2026-09-09'),1); // 09-15 is today+6 -> in; 09-08 is past -> out
