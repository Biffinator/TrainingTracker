...
function save(){if(blocked)return false;try{localStorage.setItem(KEY,JSON.stringify(db));if(activeId){const k='hybridCloudMeta:'+activeId;const m=JSON.parse(localStorage.getItem(k)||'{"revision":0}');m.dirty=true;localStorage.setItem(k,JSON.stringify(m));}cloud?.dirty();$('saved').textContent='V3.7.0 · Saved on this device at '+new Date().toLocaleTimeString();return true;}catch(e){message('Could not save to this browser. Export a backup now to keep your latest changes.');return false;}}
...
function render(){
 calendar();$('start').value=db.start;
 const c=cycle(selected,db.start),tasks=plan(selected,db.start,db.days),r=db.days[selected]||{done:{}};
 const disabled=editable()?'':'disabled';
 $('cycle').textContent=c?.partial?'STARTER WEEK':c?`CYCLE ${c.number} / WEEK ${c.week} · ${['Base','VO₂max','Tempo','Deload'][c.week-1]}`:'BEFORE YOUR CYCLE';
 ...
 $('tasks').querySelectorAll('[data-plunge]').forEach(el=>el.oninput=()=>{if(!editable()||!el.validity.valid)return;const r=rec();r.plunges||={};r.plunges[el.dataset.plunge]||=[];r.plunges[el.dataset.plunge][Number(el.dataset.session)]||={};r.plunges[el.dataset.plunge][Number(el.dataset.session)][el.dataset.field]=el.value;save();});
...
function openEditor(id=null){if(blocked)return;const t=plan(selected,db.start,db.days).find(t=>t.id===id);editingId=id;editingDate=selected;$('edit-name').value=t?label(t):'';$('edit-optional').checked=t?.optional??true;$('edit-heading').textContent=id?'Edit workout':'Add a workout';$('edit-error').textContent='';$('workout-editor').showModal();}
$('add-workout').onclick=()=>openEditor();
$('edit-cancel').onclick=()=>$('workout-editor').close();
$('edit-form').onsubmit=e=>{e.preventDefault();if(blocked||editingDate!==selected)return;const name=$('edit-name').value.trim();if(!name){$('edit-error').textContent='Enter a workout type.';return;}const r=rec();r.tasks||=structuredClone(plan(selected,db.start,db.days));if(editingId){const t=r.tasks.find(t=>t.id===editingId);t.n=name;t.optional=$('edit-optional').checked;}else r.tasks.push({id:'custom-'+crypto.randomUUID(),n:name,lift:false,optional:$('edit-optional').checked,ex:[]});save();$('workout-editor').close();render();};
