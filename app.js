import {deletePlunge,removeActivity} from './deletion.js?v=3.19.2';
import {mountPlunge} from './plunge-ui.js?v=3.19.2';
let refreshPlunge=()=>{},refreshReport=()=>{};
import {connectCloud} from './cloud.js?v=3.19.2';
import {today,weekday,addDays,cycle,plan,status,setCount,previous,migrate,validDate,validateBackup,replaceTask,resolveLongDay,weekSaturday} from './core.js?v=3.19.2';
import {isExercise} from './wellness.js?v=3.19.2';
import {mountReporting} from './reporting-ui.js?v=3.19.2';
import {mountSuggestions} from './suggestions-ui.js?v=3.19.2';
import {mergeAthletica} from './athletica.js?v=3.19.2';
...
function save(){if(blocked)return false;try{localStorage.setItem(KEY,JSON.stringify(db));if(activeId){const k='hybridCloudMeta:'+activeId;const m=JSON.parse(localStorage.getItem(k)||'{"revision":0}');m.dirty=true;localStorage.setItem(k,JSON.stringify(m));}cloud?.dirty();$('saved').textContent='V3.19.2 · Saved on this device at '+new Date().toLocaleTimeString();return true;}catch(e){message('Could not save to this browser. Export a backup now to keep your latest changes.');return false;}}
...
function openActivityLog(id,t){logId=id;logDate=selected;const existing=rec().sessions?.[id];$('log-activity-name').textContent=label(t);$('log-minutes').value=existing?.minutes??'';$('log-activity-error').textContent='';$('log-activity').showModal();}
$('log-activity-cancel').onclick=()=>$('log-activity').close();
$('log-activity-form').onsubmit=e=>{e.preventDefault();if(blocked||logDate!==selected){$('log-activity-error').textContent='This date changed. Close and try again.';return;}const minutes=$('log-minutes').value;if(minutes===''||+minutes<0){$('log-activity-error').textContent='Enter actual minutes (0 or more).';return;}const r=rec();r.sessions||={};r.sessions[logId]={minutes};r.done[logId]=true;r.missed=false;save();$('log-activity').close();render();};
