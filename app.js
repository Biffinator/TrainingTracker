import {deletePlunge,removeActivity} from './deletion.js?v=3.29.0';
import {mountPlunge} from './plunge-ui.js?v=3.29.0';
let refreshPlunge=()=>{},refreshReport=()=>{};
import {connectCloud} from './cloud.js?v=3.29.0';
import {today,weekday,addDays,cycle,plan,status,setCount,previous,migrate,validDate,validateBackup,replaceTask,resolveLongDay} from './core.js?v=3.29.0';
import {isExercise} from './wellness.js?v=3.29.0';
import {mountReporting} from './reporting-ui.js?v=3.29.0';
import {shiftMonths} from './reporting.js?v=3.29.0';
import {mountSuggestions} from './suggestions-ui.js?v=3.29.0';
import {mergeAthletica} from './athletica.js?v=3.29.0';
import {applyStrava} from './strava.js?v=3.29.0';
import {sportOf} from './wellness.js?v=3.29.0';
...
function save(){if(blocked)return false;try{localStorage.setItem(KEY,JSON.stringify(db));if(activeId){const k='hybridCloudMeta:'+activeId;const m=JSON.parse(localStorage.getItem(k)||'{"revision":0}');m.dirty=true;localStorage.setItem(k,JSON.stringify(m));}cloud?.dirty();$('saved').textContent='V3.29.0 · Saved on this device at '+new Date().toLocaleTimeString();return true;}catch(e){message('Could not save to this browser. Export a backup now to keep your latest changes.');return false;}}
