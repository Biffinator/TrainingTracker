...
export function removeActivity(record,tasks,id){
 ...
 record.tasks=structuredClone(tasks.filter(t=>t.id!==id).map(t=>({...t,ex:t.ex||[]})));delete record.done[id];if(record.sets)delete record.sets[id];if(record.sessions)delete record.sessions[id];if(record.plunges)delete record.plunges[id];return true;
}
