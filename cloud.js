...
export function connectCloud(hooks){
 ...
 document.addEventListener('input',e=>{if(session&&e.target.matches('#tasks input,#tasks select,#notes,#start,#edit-form input,#edit-form textarea,#edit-form select'))idle.touch();});
 document.addEventListener('keydown',e=>{if(session&&e.target.matches('#tasks input,#tasks select,#notes,#start,#edit-form input,#edit-form textarea'))idle.touch();});setInterval(()=>{if(!document.hidden)sync();},30000);
