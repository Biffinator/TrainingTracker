...
export function sessions(db){return Object.entries(db.days).flatMap(([date,r])=>Object.entries(r.plunges||{}).flatMap(([task,list])=>list.map((s,index)=>s?({...s,date,task,index,total:Number(s.minutes||0)*60+Number(s.seconds||0)}):null).filter(Boolean))).filter(s=>s.total>0).sort((a,b)=>(b.date+(b.time||'')).localeCompare(a.date+(a.time||'')));}
