import {plan,resolveLongDay} from './core.js?v=3.16.0';
import {clockState,duration} from './plunge.js?v=3.16.0';
export function mountPlunge(h){
 ...
 const panel=document.createElement('section');panel.className='card';panel.id='plunge-history';panel.innerHTML=`<h2>Cold plunge</h2><p class="muted">Use the timer on today's Cold plunge task in the Day view to record a live session, or log a past one below. History, stats, and charts have moved to the Reporting tab.</p><button id="manual-plunge">Add past session</button>`;document.querySelector('footer').before(panel);
